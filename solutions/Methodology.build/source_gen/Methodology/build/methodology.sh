#!/bin/sh
# ---------------------------------------------------------------------
# Methodology startup script.
# ---------------------------------------------------------------------
# Generated by MPS

message()
{
  TITLE="Cannot start Methodology"
...
# Try (in order): $Methodology_JDK, .../methodology.jdk, .../jbr, .../jbr-x86, $JDK_HOME, $JAVA_HOME, "java" in $PATH.
# ---------------------------------------------------------------------
IDEA_VENDOR_NAME='null'
# shellcheck disable=SC2154
if [ -n "$Methodology_JDK" ] && [ -x "$Methodology_JDK/bin/java" ]; then
  JDK="$Methodology_JDK"
fi

if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/Methodology1.0/methodology.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/Methodology1.0/methodology.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  ...
  message "No JDK found. Please validate either Methodology_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  fi
  exit 1
fi
...
# ---------------------------------------------------------------------
# shellcheck disable=SC2154
if [ -n "$Methodology_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$Methodology_PROPERTIES"
fi
...
elif [ -r "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/Methodology1.0/methodology$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/Methodology1.0/methodology$BITS.vmoptions"
elif [ -r "$IDE_BIN_HOME/methodology$BITS.vmoptions" ]; then
  # default, standard installation
  VM_OPTIONS_FILE="$IDE_BIN_HOME/methodology$BITS.vmoptions"
else
  # default, universal package
  test "$OS_TYPE" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
  VM_OPTIONS_FILE="$IDE_BIN_HOME/$OS_SPECIFIC/methodology$BITS.vmoptions"
fi
...
# shellcheck disable=SC2154
if [ -n "$Methodology_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$Methodology_CLASSPATH"
fi
...
IDEA_PATHS_SELECTOR=Methodology1.0
"$JAVA_BIN" \
  ${AGENT} \
  -classpath "$CLASSPATH" \
  ${VM_OPTIONS} \
  "-XX:ErrorFile=$HOME/java_error_in_Methodology_%p.log" \
  "-XX:HeapDumpPath=$HOME/java_error_in_Methodology.hprof" \
